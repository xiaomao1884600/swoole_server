...
class Adldap extends Arr
{
	...
	public function group_add_group($parent, $child)
	{
		...
		$parent_group = $this->group_info($parent, array(
			"cn"
		));
		...
		$child_group = $this->group_info($child, array(
			"cn"
		));
	...
	public function group_add_user($group, $user, $isGUID = false)
	{
		...
		// Find the group's dn
		$group_info = $this->group_info($group, array(
			"cn"
		));
	...
	public function groups_in_group($group, $recursive = NULL)
	{
		...
		$info = $this->group_info($group, array(
			"member", 
			"cn"
		));
	...
	public function recursive_groups($group)
	{
		...
		$groups = $this->group_info($group, array(
			"memberof"
		));
	...
	public function user_groups($username, $recursive = NULL, $isGUID = false)
	{
		...
		$info = @$this->user_info($username, array(
			"memberof", 
			"primarygroupid"
		), $isGUID);
	...
	public function user_password_expiry($username, $isGUID = false)
	{
		...
		$sr = ldap_read($this->_conn, $this->_base_dn, 'objectclass=*', array(
			'maxPwdAge'
		));
		if (!$sr)
		{
     ...
     * 根据用户编号/姓名获取域中用户信息
     *
     * @param string $description Search parameter 用户编号
     * @param string $search Search parameter
     * @param bool $include_desc Return a description of the user
     * @param bool $sorted Sort the user accounts
     * @return array
     */
    public function get_user_by_description($description = '', $search = "*")
    {
        if (!$this->_bind || !$description)
        {
            return false;
        }
        // Perform the search and grab all their details
        $filter = "(&(objectClass=user)(samaccounttype=" . ADLDAP_NORMAL_ACCOUNT . ")(objectCategory=person)(cn=" . $search . ")(description=" . $description . "))";
        $fields = array(
            "displayname", // 姓名
            "description", // 用户编号
            "mail", // 邮箱
        ...
        $sr = ldap_search($this->_conn, $this->_base_dn, $filter, $fields);
        $entries = ldap_get_entries($this->_conn, $sr)[0];
        $userinfo = array();
        if($entries)
        {
            $userinfo['displayname'] = $entries['displayname'][0];
            $userinfo['samaccountname'] = $entries['samaccountname'][0];
            $userinfo['description'] = $entries['description'][0];
            $userinfo['mail'] = $entries['mail'][0];
            $userinfo['department'] = $entries['department'][0];
        }
	...
	public function contact_groups($distinguishedname, $recursive = NULL)
	{
		...
		// Search the directory for their information
		$info = @$this->contact_info($distinguishedname, array(
			"memberof", 
			"primarygroupid"
		));
	...
	public function contact_ingroup($distinguisedname, $group, $recursive = NULL)
	{
		...
		// Get a list of the groups
		$groups = $this->contact_groups($distinguisedname, array(
			"memberof"
		), $recursive);
	...
	public function folder_list($folder_name = NULL, $dn_type = ADLDAP_FOLDER, $recursive = NULL, $type = NULL)
	{
		...
		if ($recursive === true)
		{
			$sr = ldap_search($this->_conn, $searchou, $filter, array(
				'objectclass', 
				'distinguishedname', 
				'samaccountname'
			));
		...
		{
			$sr = ldap_list($this->_conn, $searchou, $filter, array(
				'objectclass', 
				'distinguishedname', 
				'samaccountname'
			));
	...
	public function computer_ingroup($computer_name, $group, $recursive = NULL)
	{
		...
		//get a list of the groups
		$groups = $this->computer_groups($computer_name, array(
			"memberof"
		), $recursive);
	...
	public function computer_groups($computer_name, $recursive = NULL)
	{
		...
		//search the directory for their information
		$info = @$this->computer_info($computer_name, array(
			"memberof", 
			"primarygroupid"
		));
		$groups = $this->nice_names($info[0]["memberof"]); //presuming the entry returned is our guy (unique usernames)
	...
	public function exchange_add_X400($username, $country, $admd, $pdmd, $org, $surname, $givenname, $isGUID = false)
	{
		...
		// Find the dn of the user
		$user = $this->user_info($username, array(
			"cn", 
			"proxyaddresses"
		), $isGUID);
		if ($user[0]["dn"] === NULL)
	...
	public function exchange_contact_mailenable($distinguishedname, $emailaddress, $mailnickname = NULL)
	{
		...
		{
			// Find the dn of the user
			$user = $this->contact_info($distinguishedname, array(
				"cn", 
				"displayname"
			));
	...
	public function exchange_servers($attributes = array('cn','distinguishedname','serialnumber'))
	{
		...
		$configurationNamingContext = $this->get_root_dse(array(
	...
	{
		$namingContext = $this->get_root_dse(array(
